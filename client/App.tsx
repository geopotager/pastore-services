...
export default function App() {
  ...
  // NOUVEAU: On sépare les URLs de prévisualisation des vrais fichiers
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      // On charge les requêtes au démarrage si user connecté
      refreshRequests();
    }
  ...
  const renderSplash = () => (
    <div className="fixed inset-0 bg-neo-yellow flex flex-col items-center justify-center z-50 overflow-hidden">
      <div className="bg-white border-4 border-black p-8 shadow-neo-lg animate-[bounceIn_0.8s_ease-out_forwards] relative flex flex-col items-center">
        <h1 className="text-5xl font-black uppercase tracking-tighter text-black mb-1 animate-[slideUp_0.6s_ease-out_forwards]">
          Pastore
        </h1>
            ...
            return (
              <NeoCard key={service.id} className="flex items-center gap-4 hover:bg-gray-50 transition-colors cursor-pointer active:translate-y-1" 
                onClick={() => {
                  ...
                  setView('SERVICE_INFO');
                }}
              >
                <div className={`p-3 border-2 border-black ${service.color}`}>
  ...
  const renderServiceInfo = () => {
    ...
    return (
      <div className="space-y-6">
        <button onClick={() => setView('HOME')} className="flex items-center gap-2 font-bold hover:underline mb-2">
          <ChevronLeft size={20} /> Retour
        </button>
        
        <div className={`border-2 border-black shadow-neo p-6 ${service.color} flex flex-col items-center text-center gap-4`}>
          <div className="bg-white p-4 border-2 border-black rounded-full shadow-neo-sm">
            <Icon size={48} />
          </div>
  ...
  const renderLogin = () => (
    <div className="flex flex-col min-h-[60vh] justify-center">
      <NeoCard className="space-y-6 bg-white py-8">
        ...
        {loginError && <p className="text-red-500 text-center font-bold bg-red-100 p-2 border border-red-500">{loginError}</p>}
        ...
        <form onSubmit={async (e) => {
          ...
          try {
            if (isRegistering) {
              const newUser = await ApiService.register(name, email, phone, password);
              setUser(newUser);
            } else {
          ...
          {isRegistering && (
             <NeoInput name="phone" label="Téléphone" type="tel" placeholder="0470..." required />
          )}
          <NeoInput name="password" label="Mot de passe" type="password" placeholder="******" required />
          
          <NeoButton type="submit" size="lg" className="w-full mt-4">
            {isRegistering ? "S'inscrire" : "Accéder à l'espace"}
          </NeoButton>
        </form>
        
        <div className="text-center space-y-2">
          <button onClick={() => { setIsRegistering(!isRegistering); setLoginError(''); }} className="text-sm font-bold text-neo-blue hover:underline block w-full">
  ...
  const renderWizardCat = () => (
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-4">
        <button onClick={() => setView('HOME')} className="p-1 border-2 border-black bg-white shadow-neo-sm"><ChevronLeft/></button>
        <h2 className="font-black text-xl uppercase">Étape 1/4: Service</h2>
      </div>
        ...
        {SERVICES.map((s) => {
          ...
          return (
            <button 
              key={s.id}
              onClick={() => {
                setDraftRequest({ ...draftRequest, category: s.id });
                setView('WIZARD_DETAILS');
              }}
              className={`p-4 border-2 border-black shadow-neo flex flex-col items-center gap-2 transition-transform active:scale-95 hover:bg-opacity-80 ${s.color}`}
            >
              <Icon size={32} />
              <span className="font-bold text-sm text-center">{s.title}</span>
            </button>
          )
        })}
      </div>
  ...
  const renderWizardDetails = () => (
    <div className="space-y-6">
      ...
      <NeoTextArea 
        ...
        placeholder="Ex: Mon robinet fuit, il faut remplacer le joint..."
          ...
          <label className="flex-shrink-0 w-24 h-24 border-2 border-dashed border-black bg-gray-100 flex flex-col items-center justify-center cursor-pointer hover:bg-gray-200 transition-colors">
            ...
            <input type="file" className="hidden" accept="image/*" multiple onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
              if(e.target.files) {
                ...
                // 1. Sauvegarde des fichiers bruts pour l'upload
                setSelectedFiles([...selectedFiles, ...newFiles]);
                
                // 2. Création des URLs pour l'affichage
          ...
          {previewPhotos.map((photo, idx) => (
            <div key={idx} className="relative flex-shrink-0 w-24 h-24 border-2 border-black shadow-neo-sm">
              ...
              <button 
                ...
                className="absolute -top-2 -right-2 bg-red-500 border-2 border-black text-white p-0.5 rounded-full"
              >
                <X size={12} />
              </button>
            </div>
      ...
      <NeoButton 
        className="w-full" 
        disabled={!draftRequest.description}
        onClick={() => setView('WIZARD_CONTACT')}
      >
  ...
  const renderWizardContact = () => (
    <div className="space-y-6">
      ...
      <NeoButton 
        ...
        onClick={() => setView('WIZARD_PLANNING')}
      >
        Suivant <ArrowRight size={18} />
      </NeoButton>
    </div>
    ...
    <div className="space-y-6">
      ...
      <NeoCard color="bg-white">
        <div className="flex items-center gap-2 mb-4 border-b-2 border-gray-200 pb-2">
          <Calendar size={20} />
          <h3 className="font-bold">Vos disponibilités</h3>
        </div>
        
        {/* Simple Date Picker Strip */}
          ...
          {Array.from({length: 10}).map((_, i) => {
            ...
            return (
              <button 
                ...
                className={`flex-shrink-0 w-14 h-20 border-2 border-black flex flex-col items-center justify-center transition-all
                  ${isSelected ? 'bg-neo-black text-white shadow-none translate-y-1' : 'bg-white shadow-neo-sm hover:translate-y-[-2px]'}
                `}
              >
          ...
          <div className="grid grid-cols-3 gap-2 animate-[fadeIn_0.3s]">
            {MOCK_SLOTS.map(slot => (
              <button
                ...
                className={`py-2 px-1 text-sm font-bold border-2 border-black transition-all
                  ${selectedTime === slot ? 'bg-neo-green shadow-none' : 'bg-white hover:bg-gray-50'}
                `}
              >
                {slot}
              </button>
        ...
        onClick={async () => {
          try {
              await ApiService.createRequest({
                ...
                contact: contactForm
              }, selectedFiles); // On passe les fichiers bruts ici
      ...
      <div className="space-y-4">
        {requests.length === 0 ? (
          <div className="text-center py-10 opacity-60">
            ...
            {!user && <p className="text-xs text-red-500 mt-2">Connectez-vous pour voir votre historique.</p>}
          </div>
        ) : (
          requests.map(req => {
              ...
              <NeoCard key={req.id} className="relative overflow-hidden group hover:shadow-neo-lg transition-shadow">
                <div className="flex justify-between items-start mb-2">
                   <div className="flex items-center gap-2">
                     <div className={`p-2 border-2 border-black ${serviceDef?.color || 'bg-gray-200'}`}>
                       <Icon size={18} />
                     </div>
                     <div>
                   ...
                   <span className="px-2 py-0.5 bg-yellow-200 border-2 border-black text-xs font-bold uppercase rounded-full">
                     {req.status === 'pending' ? 'En attente' : req.status}
                   </span>
                </div>
                
                <p className="text-sm border-l-4 border-gray-300 pl-2 mb-3 bg-gray-50 p-2 italic">
                  "{req.description}"
                </p>
                ...
                <div className="flex justify-between items-end">
                    <div className="flex items-center gap-2 text-sm font-bold bg-black text-white p-2">
                    <Calendar size={14} />
                    {new Date(req.booking.date).toLocaleDateString('fr-FR', {day: 'numeric', month: 'short'})} à {req.booking.time}
                    </div>

                    <button 
                        onClick={(e) => { e.stopPropagation(); handleDelete(req.id); }}
                        className="bg-red-500 text-white p-2 border-2 border-black shadow-neo-sm hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all cursor-pointer z-10"
                    >
                        <Trash2 size={18} />
                    </button>
                </div>
  ...
  return (
    <div className="min-h-screen bg-[#f0f0f0] pb-24 font-sans text-neo-black relative overflow-x-hidden">
          ...
          <div className="absolute top-[60px] right-0 w-64 bg-white border-l-2 border-b-2 border-black shadow-neo p-4 flex flex-col gap-4 animate-[slideInView_0.2s_ease-out]">
             {user ? (
               <div className="border-b-2 border-black pb-2 mb-2">
                 <p className="text-xs text-gray-500 font-bold uppercase">Bonjour</p>
                 <p className="font-bold truncate">{user.name}</p>
               </div>
             ) : (
               <NeoButton size="sm" onClick={() => { setView('LOGIN'); setIsMenuOpen(false); }}>
                 Se connecter
               </NeoButton>
             )}
             
             <button onClick={() => { setView('HOME'); setIsMenuOpen(false); }} className="flex items-center gap-2 font-bold hover:text-neo-blue">
             ...
             {user && (
               <button onClick={handleLogout} className="text-red-500 font-bold mt-auto pt-4 border-t-2 border-gray-100">
                 Déconnexion
               </button>
             )}
          </div>
      ...
      <main className="p-4 max-w-md mx-auto min-h-[80vh]">
        <div key={view} className="animate-[slideInView_0.4s_cubic-bezier(0.16,1,0.3,1)]">
            {view === 'HOME' && renderHome()}
            {view === 'SERVICE_INFO' && renderServiceInfo()}
            {view === 'LOGIN' && renderLogin()}
            {view === 'WIZARD_CAT' && renderWizardCat()}
      ...
      <footer className="bg-neo-black text-white p-6 pb-24 mt-8 border-t-4 border-neo-yellow">
        <div className="max-w-md mx-auto text-center space-y-4">
            ...
            <p className="text-xs text-gray-500 mt-4 font-mono">
                &reg; 2024 Pastore Services MRH. <br/> Tous droits réservés.
            </p>
        </div>
      </footer>

      {/* Bottom Floating Action Button (Mobile First) */}
      {view === 'HOME' && (
        <div className="fixed bottom-6 right-6 z-40">
           <button 
             onClick={startRequest}
             className="bg-neo-black text-white w-14 h-14 rounded-full border-2 border-white shadow-neo-lg flex items-center justify-center animate-[bounce_2s_infinite]"
           >
             <Plus size={32} />
           </button>
        </div>
